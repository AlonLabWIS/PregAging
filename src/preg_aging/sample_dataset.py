...
def resample_per_age(lab_names: list[str], num_samples_per_age=20, min_quant_step=0.02, tail_ignore=0.01,
                     range_to_fac: dict[tuple[int, int], int] = None) -> (pd.DataFrame, pd.Series):
    """
    ...
    :return: The regression table with column names as lab names and the matching label (age) series.
    """
    ...
    for j, lab_name in enumerate(lab_names):
        quant_val_df = age_to_quant_and_vals(lab_name, ages=ages, min_quant_step=min_quant_step,
                                             ignore_tail=tail_ignore)
        for age in ages:
            quant = quant_val_df[age]["quant"]
            val = quant_val_df[age]["value"]
            row_indices = np.argwhere(labels == age).squeeze()
            lab_sample = LinearInterpDistribution(val, quant).rvs(size=row_indices.size)
...
def normalize_by_sub_div(regression_table):
    ...
    :return: The z-scored regression table
    """
